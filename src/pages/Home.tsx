...
const Home = () => {
  ...
  const searchValue: any = useSelector<any>(
    (state) => state.filterSlice.searchValue
  );
  const categoryType: any = useSelector<any>(
    (state) => state.filterSlice.categoryId
  );
  const sort: any = useSelector<any>((state) => state.filterSlice.sort);
  const currentPage: any = useSelector<any>(
    (state) => state.filterSlice.pageCount
  );
  ...
  const pizzas = items.map((el: any, index: number) => (
    <PizzaBlock
      key={index}
      param={""}
      count={1}
  ...
  const skeletons = [...new Array(6)].map((b, index) => (
    <Skeleton key={index} />
  ));
  return (
    <>
